 ...
 */
global $MY_SELF, $PHP_SELF, $MY_SELF_WO_SORT, $MY_REQUEST, $VERSION;
...
{
    global $MYREQUEST, $MY_SELF, $PHP_SELF;
...
<style><!--
    ...
    * html h1.apc {
    ...
    _div.info table th a.sortable {
        color: black;
    }

    _div.info table tr.tr-0 {
        background: rgb(238, 238, 238);
    }

    _div.info table tr.tr-1 {
    ...
    _div.info table td.td-0 {
        border-right: solid rgb(102, 102, 153) 1px;
        white-space: nowrap;
    }

    _div.info table td.td-n {
        border-right: solid rgb(102, 102, 153) 1px;
    }
    ...
    _div.graph table td.td-0 {
        background: rgb(238, 238, 238);
    }

    _div.graph table td.td-1 {
        background: rgb(221, 221, 221);
    }
...
// -----------------------------------------------
    case OB_HOST_STATS:
        ...
        $req_rate_user = sprintf(
            "%.2f",
            ($cache_user['num_hits'] + $cache_user['num_misses']) / ($time - $cache_user['start_time'])
        );
...
EOB;
    ...
    <tr class=tr-0><td class=td-0>Cached Variables</td><td>$number_vars ($size_vars)</td></tr>
		...
		<div class="info div2"><h2>Runtime Settings</h2><table class="table table-striped"  cellspacing=0><tbody>
EOB;

        $j = 0;
        foreach (ini_get_all('apc') as $k => $v) {
            echo "<tr class=tr-$j><td class=td-0>", $k, "</td><td>", str_replace(
                ',',
                ',<br />',
                $v['local_value']
            ), "</td></tr>\n";
...
EOB;
        ...
        '<td class=td-1><span class="green box">&nbsp;</span>Hits: ', $cache['num_hits'] . sprintf(
                " (%.1f%%)",
                $cache['num_hits'] * 100 / ($cache['num_hits'] + $cache['num_misses'])
            ), "</td>\n",
        '</tr>',
        '<tr>',
        ...
        '<td class=td-1><span class="red box">&nbsp;</span>Misses: ', $cache['num_misses'] . sprintf(
                " (%.1f%%)",
                $cache['num_misses'] * 100 / ($cache['num_hits'] + $cache['num_misses'])
            ), "</td>\n";
        echo <<< EOB
		</tr>
		</tbody></table>
            ...
            foreach ($scope_list as $j => $list) {
                foreach ($cache[$list] as $i => $entry) {
                    ...
                    foreach ($entry as $k => $value) {
                        ...
                        "<td class=td-last>", (preg_match("/time/", $k) && $value != 'None') ? date(
                            DATE_FORMAT,
                            $value
        ...
        if ($list) {
            ...
            foreach ($list as $k => $entry) {
                if (!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {
                    ...
                    echo
                    '<tr class=tr-', $i % 2, '>',
                    "<td class=td-0><a href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&SH=", md5(
                        $entry[$fieldkey]
                    ), "\">", $field_value, '</a></td>',
                    ...
                    if ($entry['deletion_time']) {
                        ...
                        if ($MYREQUEST['OB'] == OB_USER_CACHE) {
                            ...
                            echo '[<a href="', $MY_SELF, '&OB=', $MYREQUEST['OB'], '&DU=', urlencode(
                                $entry[$fieldkey]
                            ), '">Delete Now</a>]';
                            echo '</td>';
                        } else {
...
EOB;
        ...
        if ($list && $i < count($list)) {
            echo "<a href=\"$MY_SELF&OB=", $MYREQUEST['OB'], "&COUNT=0\"><i>", count(
                    $list
                ) - $i, ' more available...</i></a>';
        }
...
EOB;
        ...
        if (!$rss) {
            ...
            while (list(, $v) = each($match[2])) {
                ...
                echo "<b><a href=\"http://pecl.php.net/package/APC/$ver\">" . htmlspecialchars(
                        $v,
                        ENT_QUOTES,
                        'UTF-8'
                    ) . "</a></b><br><blockquote>";
