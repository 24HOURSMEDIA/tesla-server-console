...
.menu {overflow: hidden; display:block;height:32px;}
...
_div.info table th a.sortable { color:black; }
_div.info table tr.tr-0 { background:rgb(238,238,238); }
_div.info table tr.tr-1 { background:rgb(221,221,221); }
_div.info table td { padding:0.3em 1em 0.3em 1em; }
_div.info table td.td-0 { border-right:solid rgb(102,102,153) 1px; white-space:nowrap; }
_div.info table td.td-n { border-right:solid rgb(102,102,153) 1px; }
_div.info table td h3 {
	color:black;
	font-size:1.1em;
	margin-left:-0.3em;
	}
...
_div.graph { margin-bottom:1em }
_div.graph h2 { background:rgb(204,204,204);; color:black; font-size:1em; margin:0; padding:0.1em 1em 0.1em 1em; }
_div.graph table { border:solid rgb(204,204,204) 1px; color:black; font-weight:normal; width:100%; }
_div.graph table td.td-0 { background:rgb(238,238,238); }
_div.graph table td.td-1 { background:rgb(221,221,221); }
_div.graph table td { padding:0.2em 1em 0.4em 1em; }

div.div1,div.div2 { margin-bottom:1em; width:35em; }
div.div1 { position:relative;  top:1em; }

div.div3 { position:absolute; left:40em; top:5em; width:580px; }
...
if ($p = strpos($PHP_SELF, '?')) {
    $PHP_SELF = substr($PHP_SELF, 0, $p);
}
$PHP_SELF = $PHP_SELF . '?';
$time = time();
// sanitize _GET
    ...
    switch ($_GET['IMG']) {
        ...
        case 2: // hit miss
            ...
            fill_box(
                ...
                $size,
                50,
                -max(4, ($total - $hits) * ($size - 21) / $total),
                $col_black,
                $col_red,
...
switch ($_GET['op']) {
    ...
    case 1: // host stats
		...
		<table class="table table-striped" cellspacing=0><tbody>
...
EOB;
        ...
        echo "</td></tr>\n";
        echo "<tr class=tr-1><td class=td-0>Total Memcache Cache</td><td>" . bsize(
                $memcacheStats['limit_maxbytes']
            ) . "</td></tr>\n";
...
EOB;
        foreach ($MEMCACHE_SERVERS as $server) {
            echo '<table class="table table-striped" cellspacing=0><tbody>';
            echo '<tr class=tr-1><td class=td-1>' . $server . '</td><td><a href="' . $PHP_SELF . '&server=' . array_search(
                    $server,
                    $MEMCACHE_SERVERS
                ) . '&op=6">[<b>Flush this server</b>]</a></td></tr>';
            echo '<tr class=tr-0><td class=td-0>Start Time</td><td>', date(
                DATE_FORMAT,
                $memcacheStatsSingle[$server]['STAT']['time'] - $memcacheStatsSingle[$server]['STAT']['uptime']
            ), '</td></tr>';
            echo '<tr class=tr-1><td class=td-0>Uptime</td><td>', duration(
                $memcacheStatsSingle[$server]['STAT']['time'] - $memcacheStatsSingle[$server]['STAT']['uptime']
            ), '</td></tr>';
            echo '<tr class=tr-0><td class=td-0>Memcached Server Version</td><td>' . $memcacheStatsSingle[$server]['STAT']['version'] . '</td></tr>';
            echo '<tr class=tr-1><td class=td-0>Used Cache Size</td><td>', bsize(
                $memcacheStatsSingle[$server]['STAT']['bytes']
            ), '</td></tr>';
            echo '<tr class=tr-0><td class=td-0>Total Cache Size</td><td>', bsize(
                $memcacheStatsSingle[$server]['STAT']['limit_maxbytes']
            ), '</td></tr>';
            echo '</tbody></table>';
        }
        echo <<<EOB
...
EOB;
        ...
        graphics_avail() ?
            ...
            : "",
        '<tr>',
        '<td class=td-0><span class="green box">&nbsp;</span>Free: ', bsize($mem_avail) . sprintf(
                " (%.1f%%)",
                $mem_avail * 100 / $mem_size
            ), "</td>\n",
        '<td class=td-1><span class="green box">&nbsp;</span>Hits: ', $hits . sprintf(
                " (%.1f%%)",
                $hits * 100 / ($hits + $misses)
            ), "</td>\n",
        '</tr>',
        '<tr>',
        '<td class=td-0><span class="red box">&nbsp;</span>Used: ', bsize($mem_used) . sprintf(
                " (%.1f%%)",
                $mem_used * 100 / $mem_size
            ), "</td>\n",
        '<td class=td-1><span class="red box">&nbsp;</span>Misses: ', $misses . sprintf(
                " (%.1f%%)",
...
EOB;
    ...
    case 2: // variables
        ...
        foreach ($items as $server => $entries) {
			...
			<div class="info"><table class="table table-striped" cellspacing=0><tbody>
			<tr><th colspan="2">$server</th></tr>
			<tr><th>Slab Id</th><th>Info</th></tr>
EOB;

            foreach ($entries as $slabId => $slab) {
                $dumpUrl = $PHP_SELF . '&op=2&server=' . (array_search(
                        $server,
                        $MEMCACHE_SERVERS
                    )) . '&dumpslab=' . $slabId;
                echo
                ...
                "<td class=td-last><b>Item count:</b> ", $slab['number'], '<br/><b>Age:</b>', duration(
                    $time - $slab['age']
                ), '<br/> <b>Evicted:</b>', ((isset($slab['evicted']) && $slab['evicted'] == 1) ? 'Yes' : 'No');
                if ((isset($_GET['dumpslab']) && $_GET['dumpslab'] == $slabId) && (isset($_GET['server']) && $_GET['server'] == array_search(
                            $server,
                            $MEMCACHE_SERVERS
                        ))
                ) {
                    ...
                    foreach ($items['ITEM'] as $itemKey => $itemInfo) {
                        ...
                        echo '<a href="', $PHP_SELF, '&op=4&server=', (array_search(
                            $server,
                            $MEMCACHE_SERVERS
                        )), '&key=', base64_encode($itemKey) . '">', $itemKey, '</a>';
                        if ($i++ % 10 == 0) {
                            echo '<br/>';
...
EOB;
        ...
        '<td><a href="', $PHP_SELF, '&op=5&server=', (int)$_GET['server'], '&key=', base64_encode(
            $theKey
        ), "\">Delete</a></td>", "</tr>";
        echo <<<EOB
			</tbody></table>
